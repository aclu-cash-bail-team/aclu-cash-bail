...
var DistributionBarCell = /*#__PURE__*/function (_Cell5) {
  ...
  function DistributionBarCell(content, className) {
    ...
    _this5.values = content["values"].filter(function (dist) {
      return dist["value"] !== 0;
    });
    _this5.tooltipName = content["name"];
  ...
  _createClass(DistributionBarCell, [{
    key: "createDistributionTable",
    value: function createDistributionTable() {
      var tooltipTable = document.createElement("div");
      tooltipTable.className = "tooltip-table";
      this.values.forEach(function (dist) {
        ...
        value.innerText = "".concat(Math.round(dist["value"] * 100) / 100, "%");
    ...
    value: function createTooltip() {
      ...
      var table = this.createDistributionTable();
    ...
    key: "render",
    value: function render() {
      _get(_getPrototypeOf(DistributionBarCell.prototype), "render", this).call(this);

      var container = document.createElement("div");
      container.className = "dist-bar-container"; // create bars for each distribution
      ...
      var numDists = this.values.length;
      var gapSize = 2,
          gapUnits = "px";
      var gapCorrection = Math.round((numDists - 1) * gapSize / numDists);
      var distWidths = this.values.map(function (dist) {
        return "calc(".concat(dist["value"], "% - ").concat(gapCorrection).concat(gapUnits, ")");
      });
      container.style.columnGap = "".concat(gapSize).concat(gapUnits);
      container.style.gridTemplateColumns = distWidths.join(" "); // configure tooltip
...
var NumberLineCell = /*#__PURE__*/function (_Cell6) {
  ...
  function NumberLineCell(content, className, data) {
    ...
    _this6 = _super6.call(this, className);
    _this6.content = content["values"];
    _this6.averages = data["averages"];
    _this6.range = data;
    _this6.vizColors = ["green", "purple"];
  ...
  _createClass(NumberLineCell, [{
    ...
    value: function render() {
      ...
      this.content.forEach(function (value, i) {
        ...
        point.className = "viz-number-line-point ".concat(_this7.vizColors[i]);
        point.style.left = "".concat((value - _this7.range["start"]) / _this7.range["end"] * 100, "%");

        _this7.element.appendChild(point);
      }); // add the vertical line denoting the average

      this.averages.forEach(function (average, i) {
        var averageLine = document.createElement("div");
        averageLine.className = "bar-average-line ".concat(_this7.vizColors[i]);
        averageLine.style.left = "".concat((average["value"] - _this7.range["start"]) / _this7.range["end"] * 100, "%");

        _this7.element.appendChild(averageLine);
      });
    }
  }]);
...
var HeaderCell = /*#__PURE__*/function (_Cell7) {
  ...
  function HeaderCell(content, className, sortCol, sortDir, initSort, table, id) {
    ...
    _this8 = _super7.call(this, className);
    _this8.content = content;
    _this8.sortCol = sortCol;
    _this8.sortDir = sortDir;
    _this8.initSort = initSort;
    _this8.table = table;
    _this8.id = id;

    _this8.render(); // add event listener for sorting


    if (_this8.sortCol) {
      _this8.element.addEventListener("click", function () {
        // always default to descending sort
        if (_this8.table.sortCol !== _this8.id) _this8.sortDir = -1;

        var classNameWithSort = _this8.getClassName();

        _this8.table.setSortColumn(_this8.id);

        _this8.table.setSortDirection(_this8.sortDir);

        _this8.table.sort(false); // after sorting set the class to ensure its the only column highlighted


        _this8.setElementClass(classNameWithSort, true); // toggle sort direction for the next click
  ...
  _createClass(BodyRow, [{
    ...
    value: function render(sorted) {
      ...
      this.cells.forEach(function (cell, i) {
        ...
        if (_this9.isBold) cell.addElementClass("bold-cell");
        row.appendChild(cell.element);
      });
      return [this.element];
    }
...
var CollapsibleBodyRow = /*#__PURE__*/function (_BodyRow) {
  ...
  function CollapsibleBodyRow(cells, outlier, collapseRows, isHidden, isCollapsed) {
    ...
    _this10 = _super9.call(this, cells, outlier, isHidden);
    _this10.isCollapsed = isCollapsed;
    _this10.collapseRows = collapseRows;
    return _this10;
  }
  ...
  _createClass(Table, [{
    ...
    value: function validate() {
      ...
      if (this.data.some(function (row) {
        return row.data.length != _this11.headers.length;
      })) {
    ...
    value: function init() {
      ...
      var searchOptions = this.data.flatMap(function (row) {
        var rowOptions = row.data.flatMap(function (value, i) {
          return _this12.searchCols[i] ? [value] : [];
        });
      ...
      searchInput.addEventListener("change", function (e) {
        ...
        _this12.searchTerms = searchValue.split(";").filter(function (s) {
          return s !== "";
        });
        _this12.rows = _this12.getRows();
      ...
      viewAllButton.addEventListener("click", function () {
        _this12.isTruncated = !_this12.isTruncated;
        viewAllButton.innerText = _this12.isTruncated ? VIEW_ALL : VIEW_LESS;
        _this12.rows = _this12.getRows();
    ...
    value: function getHeaderRow() {
      ...
      var headerCells = this.headers.map(function (header, i) {
        ...
        return new CellType(CellType === HeaderCell ? header["text"] : header, _this13.classNames[i], _this13.sortCols[i], // 1 designates ascending; -1, descending (default); 0, not sortable
        _this13.sortCols[i] ? -1 : 0, i === _this13.sortCol, _this13, i);
      });
    ...
    value: function getCells(data, isOutlier) {
      ...
      return data.map(function (cell, j) {
        ...
        return new CellType(cell, _this14.classNames[j], _this14.headers[j]);
      });
    }
  }, {
    key: "getRows",
    value: function getRows() {
      ...
      var rows = this.data.map(function (row) {
        ...
        var cells = _this15.getCells(row.data, row.outlier);

        var isRowSearched = _this15.searchTerms.some(function (searchTerm) {
          return row.data.some(function (value, i) {
            return (// Search term is selected from dropdown so
              // is guaranteed to be equal to a value
              _this15.searchCols[i] && value.toLowerCase() === searchTerm.toLowerCase()
            );
          });
        });

        var isTruncated = _this15.isTruncated && numVisibleRows >= NUM_TRUNCATED_ROWS;
        var isHiddenOutlier = row.outlier && !_this15.showOutliers;
        var isRowVisible = isRowSearched || !isTruncated && !isHiddenOutlier && _this15.searchTerms.length === 0;

        if (row.collapseData !== undefined) {
          var collapseRows = row.collapseData.map(function (collapseRow) {
            var isSubRowSearched = _this15.searchTerms.some(function (searchTerm) {
              return (// For simplicity, only the first sub-row column is searchable
                collapseRow.data[1].toLowerCase() === searchTerm.toLowerCase()
              );
            });

            var isSubRowHiddenOutlier = collapseRow.outlier && !_this15.showOutliers;
            var isSubRowVisible = isSubRowSearched || !row.isCollapsed && !isSubRowHiddenOutlier;
            return new BodyRow(_this15.getCells(collapseRow.data, collapseRow.outlier), collapseRow.outlier, !isSubRowVisible);
          });
    ...
    value: function sort(initialSort) {
      ...
      this.data.sort(function (a, b) {
        var val1 = a.data[_this16.sortCol];
        var val2 = b.data[_this16.sortCol]; // Determine if value is treated as a number or a string

        var i = _this16.getSortable(val1);

        var j = _this16.getSortable(val2);

        if (i < j) {
          return _this16.sortDir * -1;
        } else if (i > j) {
          return _this16.sortDir;
        } else {
          return 0;
        }
      });
    ...
    value: function render() {
      ...
      if (!this.isVisible) {
        ...
        this.rows.forEach(function (row, i) {
          var domNodes = row.render(_this17.sortCol);
          ...
          if (row instanceof CollapsibleBodyRow) {
            row.element.addEventListener("click", function () {
              _this17.data[i].isCollapsed = !row.isCollapsed;
              _this17.rows = _this17.getRows();
...
createMdjTable();
},{"./classes/Table.js":"src/js/classes/Table.js","./data.js":"src/js/data.js"}],"node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
